...
CREATE POLICY "driver_requests: solo superadmin borra"
ON public.driver_requests FOR DELETE USING (public.is_superadmin());
...
-- ============================================================
-- TAXI SEGURO — migraciones y limpieza
-- ============================================================
-- Las tablas `drivers` y `driver_requests`, sus índices y sus políticas se definen UNA sola vez, más arriba.
-- Antes había aquí una segunda copia con detalles distintos (el CREATE TABLE IF NOT EXISTS de esa copia nunca
-- hacía nada) y con dos políticas de más que repetían otras con distinto nombre; se quitan de las bases que ya las tenían:
DROP POLICY IF EXISTS "driver_requests: insert público"        ON public.driver_requests;   -- = «cualquiera se postula»
DROP POLICY IF EXISTS "driver_requests: solo superadmin edita" ON public.driver_requests;   -- = «solo superadmin gestiona»
