...
/**
 * Plantilla "Ficha Digital".
 *
 * Su inicio es una ficha de negocio (portada, logo, horario, dirección,
 * delivery, botones de Teléfono/WhatsApp) seguida de accesos circulares por
 * categoría, un buscador y el listado de productos agrupado — el formato que
 * suelen vender como "reemplazo del PDF de carta". Comparte motor (catálogo,
      ...
      <main className="pt-16 md:pt-[60px] pb-24 md:pb-12">
        ...
        {activeTab === 'home' && (
          <div className="animate-fade-in">
            ...
            {/* ══ FICHA DE NEGOCIO ══ */}
            <div className="px-5 md:px-6 -mt-14 relative z-20 max-w-2xl md:mx-auto">
              <div className="rounded-3xl shadow-xl border p-5 md:p-7" style={{ background: t.surface, borderColor: `${t.outlineVariant}40` }}>

                {/* Logo + nombre — solo el logo sube sobre la portada; el texto se queda dentro de la tarjeta */}
                ...
                {categoriasCirculo.length > 0 && (
                  <div className="grid grid-cols-3 gap-2 mt-5">
                    {categoriasCirculo.slice(0, 3).map((cat) => (
                      <button key={cat.id} onClick={() => navToMenu(cat.id)} className="aspect-square rounded-xl overflow-hidden">
                        <img className="w-full h-full object-cover hover:scale-105 transition-transform duration-300" alt={cat.label} src={cat.image} />
                      </button>
                    ))}
                  </div>
                )}
